<template>
  <div>
      ...
      <b-row class="row-spacing" v-for="(row, rowI) in numRows" :key="rowI">
        ...
        >
          <AllergyCard
            ...
            :cardType="`allergy`"
            v-on:remove-allergy="removeAllergy"
          />
        </b-col>
      </b-row>
      <h1 v-if="pageAllergies.empty">No Allergies Found.</h1>
      <b-pagination
        v-model="page"
    ...
    <h1 v-else>No Allergies Found.</h1>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    removeAllergy: function (medicine) {
      this.$confirm({
        title: "Remove Allergy",
        message: `Are you sure you want to remove drug ${medicine.name} from allergies?`,
        button: {
          no: "No",
          yes: "Yes",
        },
        callback: (confirm) => {
          if (confirm) {
            PatientService.removeAllergy(this.loggedUser().id, medicine.id)
              .then((response) => {
    ...
    loadPage: function () {
      PatientService.getAllergiesPatient(
        this.loggedUser().id,
